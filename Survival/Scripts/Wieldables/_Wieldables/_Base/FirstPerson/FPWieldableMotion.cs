...
namespace SurvivalTemplatePro.WieldableSystem
{
	...
	public class FPWieldableMotion : STPWieldableEventsListener<FPWieldableMotion>
	{
		#region Internal
		[System.Serializable]
		...
		protected override void Awake()
        {
			...
			m_PositionSpring = new Spring(Spring.RefreshType.OverridePosition, m_Pivot, m_Pivot.transform.localPosition, m_SpringLerpSpeed);
			m_RotationSpring = new Spring(Spring.RefreshType.OverrideRotation, m_Pivot, m_Pivot.transform.localEulerAngles, m_SpringLerpSpeed);
        ...
        protected override void OnInitialized(ICharacter character)
        {
			...
			character.TryGetModule(out m_Interaction);
			character.TryGetModule(out m_Mover);
		...
		private void UpdateState()
		{
			...
			else
			{
				...
				else if (Wieldable.ItemDurability != null && Wieldable.ItemDurability.Float <= 0.001f)
		...
		{
			if (!m_ViewOffset.Enabled || !m_Mover.Motor.IsGrounded || m_CustomState != null)
				return;
		...
		private void UpdateSway(ref Vector3 position, ref Vector3 rotation)
		{
			...
			position += new Vector3(
				...
				m_CharacterInput.y * m_Sway.LookPositionSway.z * -k_PosStrength) * m_CurrentState.OutsideForcesStrength;

			// Look rotation sway
			rotation += new Vector3(
				m_CharacterInput.y * m_Sway.LookRotationSway.x,
				m_CharacterInput.x * -m_Sway.LookRotationSway.y,
				m_CharacterInput.x * -m_Sway.LookRotationSway.z) * m_CurrentState.OutsideForcesStrength;
			...
			position += new Vector3(
				...
				-m_LocalCharacterVelocity.z * m_Sway.StrafePositionSway.z) * (k_PosStrength * m_CurrentState.OutsideForcesStrength);
			...
			rotation += new Vector3(
				...
				m_LocalCharacterVelocity.x * m_Sway.StrafeRotationSway.z) * (k_RotStrength * m_CurrentState.OutsideForcesStrength);
