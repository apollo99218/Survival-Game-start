...
namespace SurvivalTemplatePro.WieldableSystem
{
	...
	public class FPWieldableMotionStateDrawer : PropertyDrawer
	{
		...
		public override void OnGUI (Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (property.isExpanded)
			{
				if (Application.isPlaying)
				{
					...
					if (GUI.Button(position, "Visualize"))
					{
						if (m_MotionComponent != null && m_MotionComponent.enabled && m_MotionComponent.gameObject.activeInHierarchy)
						{
							...
							m_MotionComponent.VisualizeState(m_Visualize ? GetMotionState(property) : null, m_VisualizationSpeed);
						}
					}
				...
				if (GUI.changed && m_Visualize && m_MotionComponent != null)
					m_MotionComponent.VisualizeState(GetMotionState(property), m_VisualizationSpeed);
			}
		}
		...
		private FPWieldableMotionState GetMotionState(SerializedProperty property) 
		{
			FPWieldableMotionState motionState;

			var prop = property.serializedObject.FindProperty(property.propertyPath);
			motionState = prop.GetValue<FPWieldableMotionState>();
