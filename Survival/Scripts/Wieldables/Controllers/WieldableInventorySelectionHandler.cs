...
namespace SurvivalTemplatePro.WieldableSystem
{
    ...
    public class WieldableInventorySelectionHandler : CharacterBehaviour, IWieldableSelectionHandler
    {
        ...
        private List<WieldableItemReference> m_InventoryBasedWieldables;

#if UNITY_EDITOR
        [SerializeField, Space, FolderReference]
        ...
        {
            if (wieldableItemReference != null && !m_InventoryBasedWieldables.Contains(wieldableItemReference))
                m_InventoryBasedWieldables.Add(wieldableItemReference);
        }
        ...
        public void Refresh() => SelectAtIndex(m_SelectedIndex, 1f, true);

        public void SelectAtIndex(int indexToSelect, float holsterSpeed = 1f, bool forceEquip = false)
        {
            m_SelectedIndex = Mathf.Clamp(indexToSelect, 0, m_Holster.Count - 1);

            EquipWieldable(m_Holster[m_SelectedIndex].Item, holsterSpeed, forceEquip);
        ...
        private void EquipWieldable(Item itemToAttach, float holsterSpeed, bool forceEquip)
        {
            if (itemToAttach != null && m_Wieldables.TryGetValue(itemToAttach.Id, out IWieldable wieldable))
                m_WieldableController.TryEquipWieldable(wieldable, itemToAttach, holsterSpeed, forceEquip);
            else
                m_WieldableController.TryEquipWieldable(null, null, holsterSpeed, forceEquip);
        }
        ...
        {
            for (int i = 0; i < m_InventoryBasedWieldables.Count; i++)
            {
                if (m_InventoryBasedWieldables[i] != null)
                {
                    int itemId = m_InventoryBasedWieldables[i].ItemReference;
